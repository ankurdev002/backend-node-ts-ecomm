...
export const getCouponsSchema = z.object({
  query: z.object({
    ...
    sortBy: z.enum(["name", "createdAt", "validUntil"]).optional(),
...
// Schema for getting available coupons
export const getAvailableCouponsSchema = z.object({
  query: z.object({
    orderAmount: z
      .string()
      .regex(/^\d+(\.\d+)?$/, "Order amount must be a number"),
    productIds: z.string().optional(),
  }),
});

// Schema for coupon usage history
export const getCouponHistorySchema = z.object({
  query: z.object({
    page: z.string().regex(/^\d+$/, "Page must be a number").optional(),
    limit: z.string().regex(/^\d+$/, "Limit must be a number").optional(),
    sortBy: z.enum(["usedAt", "discountAmount"]).optional(),
    sortOrder: z.enum(["ASC", "DESC"]).optional(),
  }),
});
...
export type GetAvailableCouponsInput = z.infer<
  typeof getAvailableCouponsSchema
>;
export type GetCouponHistoryInput = z.infer<typeof getCouponHistorySchema>;
export type GetCouponStatsInput = z.infer<typeof getCouponStatsSchema>;
